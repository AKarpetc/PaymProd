...
public class MenuEditingTests : IDisposable
{
    ...
    {
        // Arrange
        var menuId = _menuRepo.CreateMenu("Test Menu", 10, "", "");
        var typeId = _dishRepo.AddDelicateType("Type");
        var dishId = _dishRepo.AddDelicate(typeId, "Original Dish", 100, 1, false);
    ...
    {
        // Arrange
        var menuId = _menuRepo.CreateMenu("Comp Test", 10, "", "");
        var typeId = _dishRepo.AddDelicateType("Type");
        var dishId = _dishRepo.AddDelicate(typeId, "Dish With Comps", 100, 1, false);

        var p1 = _prodRepo.AddProduct("Prod1", 1, 1, 1, 1);
        var p2 = _prodRepo.AddProduct("Prod2", 1, 1, 1, 1);
