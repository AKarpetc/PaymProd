...
namespace PaymProdNet9.Tests
{
    ...
    public class ProductReportTests : IDisposable
    {
        ...
        public void CalculateSummary_ShouldSumQuantitiesAndPrices_ForStandardDish()
        {
            ...
            int prodId = _prodRepo.AddProduct("Beef", 1, 1, 10, 1, price: 10);
        ...
        public void CalculateSummary_ShouldHandle_DirectProductWithPackaging()
        {
            ...
            int prodId = _prodRepo.AddProduct("Cola", 1, 1, 1, 1, price: 50); // Price 50 per unit (usually per Fass if packaged)
        ...
        {
            int prodId = _prodRepo.AddProduct("Hidden Prod", 1, 1, 1, 1);
            
            var item1 = new MenuDel_act { 
                Del_id = 1, 
                HideInProductReport = false,
                Lcomp = new List<Components> { new Components { Prodid = prodId, Ves=10 } }
            };
