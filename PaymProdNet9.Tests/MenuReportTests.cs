...
namespace PaymProdNet9.Tests
{
    ...
    public class MenuReportTests : IDisposable
    {
        ...
        public void CalculateReport_CostMode_ShouldSumIngredientPrices_NoMarkup_NoService()
        {
            // Arrange
            // Product Cost: 10
            int pId = _prodRepo.AddProduct("Prod", 1, 1, 1, 1, price: 10);

            // Dish Cost: 2 * 10 = 20
            var delicate =
                CreateDishWithComponent(pId, 2, markup: 200, count: 1); // Markup 200% should be ignored in Cost mode
        ...
        public void CalculateReport_PriceMode_ShouldApplyMarkup_AndServicePercent()
        {
            ...
            var delicate = CreateDishWithComponent(pId, 2, markup: 200, count: 1);
        ...
        public void CalculateReport_PriceMode_ShouldUseMenuSpecificServicePercent()
        {
            // Arrange
            int pId = _prodRepo.AddProduct("Prod", 1, 1, 1, 1, price: 10);
            var delicate = CreateDishWithComponent(pId, 1, markup: 100, count: 1); // Price 10 * 100% = 10

            // Create Menu with specific service 15%
            int menuId = _menuRepo.CreateMenu("Menu 15%", 10, "", "");
