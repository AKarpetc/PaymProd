...
public class DelicateRepositoryTests : IDisposable
{
    ...
    public static IEnumerable<object[]> GetFlagCombinations()
    {
        ...
        {
            var autoAdd = (i & 1) != 0;
            var hideInMenu = (i & 2) != 0;
            var hideInReport = (i & 4) != 0;
    ...
    {
        // Arrange
        var typeId = _repository.AddDelicateType("Test Type");
        var name = $"Delicate_{autoAdd}_{hideInMenu}_{hideInReport}";

        // Act
        var id = _repository.AddDelicate(typeId, name, 100, 1, autoAdd, hideInMenu, hideInReport);
    ...
    public void UpdateDelicate_ShouldUpdateFlagsCorrectly(bool autoAdd, bool hideInMenu, bool hideInReport)
    {
        // Arrange
        var typeId = _repository.AddDelicateType("Test Type");
        var id = _repository.AddDelicate(typeId, "Original Name", 100, 1, false, false, false);
    ...
    public void Components_CRUD_ShouldWork()
    {
        // Arrange
        var typeId = _repository.AddDelicateType("Test Type");
        var delId = _repository.AddDelicate(typeId, "Test Dish", 100, 1, false);

        // Create a product to use as component
        var prodId = _productRepository.AddProduct("Component Prod", 1, 1, 1, 1);
    ...
    public void DeleteDelicate_ShouldSoftDelete()
    {
        // Arrange
        var typeId = _repository.AddDelicateType("Test Type");
        var id = _repository.AddDelicate(typeId, "To Delete", 100, 1, false);
    ...
    public void RestoreDelicate_ShouldUnsetIsDeleted()
    {
        // Arrange
        var typeId = _repository.AddDelicateType("Test Type");
        var id = _repository.AddDelicate(typeId, "To Restore", 100, 1, false);
        _repository.DeleteDelicate(id);
    ...
    public void UpdateDefaultMarkup_ShouldWork()
    {
        // Arrange
        var typeId = _repository.AddDelicateType("Test Type");
        var id = _repository.AddDelicate(typeId, "Markup Test", 100, 1, false);
